...
describe '[LAB CHECKER] Feature Spec Requirements' do
  ...
  capybara_methods = %w[visit fill_in click_link click_button within have_content have_selector check select choose attach_file]
  models = %w[User Post]
  ...
  it 'collectively covers User and Post functionality across feature specs' do
    models.each do |model|
      expect(all_content.match?(/#{model}/i)).to be(true), "Expected at least one feature spec to test #{model} functionality."
    end
  end
  ...
  it 'uses FactoryBot for test data in at least one feature spec' do
    expect(all_content.match?(/(build|create)(_list|_stubbed)?\s*\(:\w+/)).to be(true), "Tip: Use FactoryBot for creating test data in feature specs."
  end

  it 'includes meaningful assertions using expect(page) or have_content/selector/current_path' do
    expect(all_content.match?(/expect\(page\)|have_content|have_selector|have_current_path/)).to be(true), "Expected feature specs to check page content or state."
  end
  ...
  feature_files.each do |file|
    ...
    it "#{File.basename(file)} uses describe, at least 2 it blocks, setup best practices, and avoids implementation details" do
      ...
      expect(content.match?(/(let\s*\(|before\s*\{)/)).to be(true)
